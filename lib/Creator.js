...
module.exports = class Creator extends EventEmitter {
  ...
  async create(cliOptions = {}, preset = null) {
    ...
    if (cliOptions.preset) {
      // wonder-mobile create foo --preset taroMT
      preset = await this.resolvePreset(cliOptions.preset, cliOptions.clone)
    } else {
      preset = await this.resolvePreset(defaults.presets.default, cliOptions.clone)
    }
    await clearConsole()
    log(chalk.blue.bold(`Wonder-mobile CLI v${require('../package.json').version}`))
    ...
    const { pkgVers, pkgDes } = await inquirer.prompt([
      ...
      },
      {
        name: 'pkgDes',
        message: `请输入项目简介`,
        default: 'project created by xiaohouye-cli',
      }
    ])
    // 将下载的临时文件拷贝到项目中
    ...
    if (!cliOptions.skipGetStarted) {
      log(
        ...
        chalk.cyan(` ${chalk.gray('$')} ${packageManager === 'yarn' ? 'yarn install' : packageManager === 'pnpm' ? 'pnpm install' : 'npm install'}`)
      )
    }
    log()
